 ...
 # --- グラフ作成コード　★★★ここからが今回の修正点★★★ ---
                    ...
                    if not events_to_plot.empty:
                        for idx, row in events_to_plot.iterrows():
                            ...
                            ax.annotate(row['イベント'], xy=(row['プロット用日時'], y_pos), xytext=(0, 10), textcoords='offset points', ha='center', va='bottom', fontsize=12, bbox=dict(boxstyle='round,pad=0.3', fc='yellow', alpha=0.7), zorder=11)
                    ...
                    ax.set_ylim(0, 100)
                    ax.set_title("軌跡シート", fontsize=16)
                    ax.set_ylabel("フェーズスコア", fontsize=16)
                    ax.set_xlabel("日付", fontsize=16)
                    ax.tick_params(axis='both', labelsize=16)
                    plt.xticks(rotation=30, ha="right")
                    if ax.get_legend_handles_labels()[0]:
                        ax.legend(title='アプリ用患者ID', fontsize=16, title_fontsize=16, loc='upper right')
                    ...
                    ax.text(0.02, 10, "超急性期", fontsize=16, transform=ax.get_yaxis_transform(), ha="left", va="center", bbox=bbox_style, zorder=10)
                    ax.text(0.02, 40, "維持期", fontsize=16, transform=ax.get_yaxis_transform(), ha="left", va="center", bbox=bbox_style, zorder=10)
                    ax.text(0.02, 70, "回復期", fontsize=16, transform=ax.get_yaxis_transform(), ha="left", va="center", bbox=bbox_style, zorder=10)
                    ax.text(0.02, 90, "転棟期", fontsize=16, transform=ax.get_yaxis_transform(), ha="left", va="center", bbox=bbox_style, zorder=10)
