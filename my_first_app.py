    ...
    # 夕方のデータを0.5日ずらすための列を追加
        ...
        # st.expanderで折りたたみセクションを作成
                ...
                # ★★★ ここから修正 ★★★
                    ...
                    st.info(
                    "このグラフは、選択された疾患群の全患者の回復曲線（半透明の線）と、その平均軌跡（赤線）を示しています。"
                    "これにより、その疾患の典型的な回復パターンと、個々の患者のばらつきを視覚的に把握できます。"
                    )
                # 分析対象の疾患群をユニークなリストとして取得
                    ...
                    if len(disease_groups) > 0:
                        ...
                        # ★★★ ここまで修正 ★★★
                            ...
                            st.info(
                            "このグラフは、スコアが1日あたり平均してどれくらい変化したかを示しています。"
                            "正の値が大きいほど回復の勢いが強く、負の値は状態の悪化を示唆します。回復が加速・停滞するタイミングを分析できます。"
                            )
                            all_changes = []
                            for patient_id in patient_ids:
                    ...
                    with tab2:
                       # ★★★ ここから修正 ★★★
                        ...
                        st.info(
                            "この箱ひげ図は、各フェーズに滞在した日数の分布を疾患群ごとに比較しています。"
                            "箱の長さが短いほど日数のばらつきが少なく、治療期間が安定していることを示唆します。治療が長引きやすいフェーズの特定に役立ちます。"
                        )
                ...
                # ★★★ ここまで修正 ★★★
                        ...
                        st.info(
                        "以下の表は、疾患群ごとの主要な臨床指標をまとめたものです。"
                        "日数は「中央値 [四分位範囲]」、率は「パーセント (該当者数/全体数)」で表示しています。"
                        )
                        # ★★★ ここからロジックを全面的に修正 ★★★
                        # --- 計算ロジック ---
                        # 1. 患者数
